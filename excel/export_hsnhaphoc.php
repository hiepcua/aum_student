...
$nameFile="hsnhaphoc_".date("YmdHis");
// Khởi tạo đối tượng mới và xử lý
$objPHPExcel = new PHPExcel();
// Set document properties
$objPHPExcel->getProperties()->setCreator("EEA")
->setLastModifiedBy("EEA")
->setTitle("EEA - Danh sách hồ sơ nhập học")
->setSubject("EEA - Danh sách hồ sơ nhập học")
->setDescription("EEA - Danh sách hồ sơ nhập học")
->setKeywords("EEA - Danh sách hồ sơ nhập học")
->setCategory("EEA - Danh sách hồ sơ nhập học");
...
while($r=$obj->Fetch_Assoc()) { 
	...
	$date=''; if($r['date_nhaphoc']!=null) $date=date("d/m/y",$r['date_nhaphoc']);
	$objPHPExcel->getActiveSheet()->setCellValue('A'.$i, $stt);
	$objPHPExcel->getActiveSheet()->getStyle('A'.$i)->applyFromArray($styleCenter);
	$objPHPExcel->getActiveSheet()->setCellValue('B'.$i, stripslashes($r['ho_dem']).' '.stripslashes($r['name']));
	$objPHPExcel->getActiveSheet()->setCellValue('C'.$i, date("d/m/y",$r['ngaysinh']));
	$objPHPExcel->getActiveSheet()->setCellValue('D'.$i, $gender);
	$objPHPExcel->getActiveSheet()->setCellValue('E'.$i, $r['dienthoai']);
	$objPHPExcel->getActiveSheet()->setCellValue('F'.$i, $r['diachi']);
	$objPHPExcel->getActiveSheet()->setCellValue('G'.$i, $_NGANH['N'.$r['id_nganh']]);
	$objPHPExcel->getActiveSheet()->setCellValue('H'.$i, $_KHOA['K'.$r['id_khoa']]);
	$objPHPExcel->getActiveSheet()->setCellValue('I'.$i, $_HE['H'.$r['id_he']]);
	$objPHPExcel->getActiveSheet()->setCellValue('J'.$i, 'Đã nhập học');
	$objPHPExcel->getActiveSheet()->setCellValue('K'.$i, $masv);
	$objPHPExcel->getActiveSheet()->setCellValue('L'.$i, $date);
	$objPHPExcel->getActiveSheet()->setCellValue('M'.$i, $r['malop']);

	$objPHPExcel->getActiveSheet()->getStyle('A'.$i.':M'.$i)->getBorders()->getAllBorders()->setBorderStyle(PHPExcel_Style_Border::BORDER_THIN);
	$stt++;$i++;

	$objPHPExcel->getActiveSheet()->getPageSetup()->setPrintArea('A1:M'.$i); 
}
